...
//getting Mens Phones and Tablets
  ...
  function cus_name(){
    ...
    $stmt=mysqli_query($conn,$get_name);
    
    while($row=mysqli_fetch_array($stmt)){
         $name=$row['fname'];

         echo "<div style='text-align:center;  color:green'><h2>Welcome:$name</h2></div>  ";
     ...
     function change_password(){
      ...
      $user= $_SESSION["Customer_Email"];
       if(isset($_POST["change_pass"])){
         ...
         $new_again=$_POST["Again_pass"];

         $get_pass="SELECT * FROM customers WHERE pass='$current_pass' AND Customer_Email='$user' ";

         $stmt=mysqli_query($conn,$get_pass);
         $check_pass=mysqli_num_rows($stmt);
         ...
         if($new_pass!=$new_again){
           ...
           $update_pass="UPDATE customers SET pass='$new_pass' WHERE Customer_Email='$user' ";
           $run_update=mysqli_query($conn,$update_pass);

           if($run_update){
             echo "<script>alert('Your Password Have been Updated Sucessfully!')</script>";
             echo "<script> window.open('customer_account.php','_self')</script>";
